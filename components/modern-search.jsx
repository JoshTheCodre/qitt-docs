...
export default function ModernSearch({ value, onChange, onSearch, showSuggestions = false }) {
    ...
    <div className="relative">
      <div className="relative">
        <div className="absolute left-4 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
          ...
          {!value && <Sparkles className="w-4 h-4 text-blue-500 animate-pulse" />}
        </div>
        <Input
          ...
          <div className="flex flex-wrap gap-2">
            {mockSuggestions.slice(0, 6).map((suggestion) => (
              <Badge
                key={suggestion}
                variant="secondary"
                className="cursor-pointer hover:bg-blue-100 hover:text-blue-700 transition-colors rounded-lg"
                onClick={() => {
                  onChange(suggestion)
                  onSearch()
                }}
